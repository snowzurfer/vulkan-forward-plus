...
InputManager::InputManager()
    : mouse_x_(0),
      ...
      mouse_down_(),
      mouse_pressed_(),
      cursor_mode_(MouseCursorMode::NORMAL) {
...
void InputManager::Init(GLFWwindow *window) {
  ...
  // Used to retrieve the input manager instance within the static callback funcs 
...
void InputManager::GLFWKeyCallback(
      GLFWwindow* window,
      int32_t key,
      int32_t scancode,
      int32_t action,
...
void InputManager::GLFWCursorPositionCallback(
      GLFWwindow* window,
      double x_pos,
      double y_pos) {
  InputManager *manager =
...
void InputManager::GLFWMouseButtonCallback(
      GLFWwindow *window,
      int32_t button,
      int32_t action,
      int32_t mods) {
  ...
  manager->HandleMouseEvent(button, action);
}
