...
static bool IsQueueFamilyIndicesComplete(
...
void VulkanDevice::Init(VkInstance instance, VkSurfaceKHR surface) {
...
#ifndef NDEBUG
  layers.assign(
      kDeviceDebugValidationLayers.begin(),
  ...
  VK_CHECK_RESULT(vkCreateDevice(physical_device_, &device_create_info, nullptr,
                                 &device_));
...
uint32_t VulkanDevice::GetMemoryType(
    uint32_t type_bits,
    VkMemoryPropertyFlags properties_flags) const {
  const uint32_t &memory_type_count = physical_memory_properties_.memoryTypeCount;
...
bool VulkanDevice::IsPhysicalDeviceSuitable(
  ...
  if (vkEnumerateDeviceExtensionProperties(physical_device, nullptr,
      ...
      != VK_SUCCESS || extensions_count == 0U) {
  ...
  std::vector<VkQueueFamilyProperties>
    queue_family_properties(queue_families_count);
...
void VulkanDevice::CreateImageView( 
  ...
  VK_CHECK_RESULT(vkCreateImageView(device_, &image_vew_create_info, nullptr, &view));
